...
class LayerNorm_fw(nn.LayerNorm):
    def __init__(self, normalized_shape, eps=1e-5, elementwise_affine=True):
        super(LayerNorm_fw, self).__init__(normalized_shape, eps, elementwise_affine)
    ...
    def forward(self, x):
        if self.weight.fast is not None and self.bias.fast is not None:
            # weight.fast (fast weight) is the temporarily adapted weight
            # weight = self.weight.fast.unsqueeze(0).unsqueeze(1)
            # bias = self.bias.fast.unsqueeze(0).unsqueeze(1)
            out = F.layer_norm(x, self.normalized_shape, self.weight.fast, self.bias.fast, self.eps)
        else:
            out = super(LayerNorm_fw, self).forward(x)
        return out
...
class Mlp(nn.Module):
    ...
    """
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, bias=True, drop=0., meta=True):
        super().__init__()
        ...
        if(meta):
            self.fc1 = Linear_fw(in_features, hidden_features, bias=bias[0])
            self.fc2 = Linear_fw(hidden_features, out_features, bias=bias[1])
        else:
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., meta=True):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, meta=True):
        super().__init__()
        self.dim = dim

        if(meta):
            norm_layer = LayerNorm_fw
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, drop=drop, meta=True)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block, meta=True): 
        """
        Args:
