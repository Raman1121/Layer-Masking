...
class LayerNorm_fw(nn.LayerNorm):
    ...
    def forward(self, x):
        if self.weight.fast is not None and self.bias.fast is not None:
            ...
            out = F.layer_norm(
                x, self.normalized_shape, self.weight.fast, self.bias.fast, self.eps
            )
        else:
...
class Attention(nn.Module):
    def __init__(
        self, dim, num_heads=8, qkv_bias=False, attn_drop=0.0, proj_drop=0.0, meta=True
    ):
        super().__init__()
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.cls_token = (
            nn.Parameter(torch.zeros(1, 1, embed_dim)) if self.num_tokens > 0 else None
        )
        self.pos_embed = nn.Parameter(
            torch.randn(1, num_patches + self.num_tokens, embed_dim) * 0.02
        )
